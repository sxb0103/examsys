package com.fqedu.examsys.filter;
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(uri.contains("userLogin")){
            ...
            if(token != null){
                ...
                String token2 = MD5Utils.md5(userName + "Nice");
                if (token != null && token.equals(token2)) {// 合法用户
                    ...
                    if(head != null && head.equals("XMLHttpRequest")){
                        resp.getWriter().write("{\"info\":\"2\"}");
                        return;
                    }else {
                        // 跳转到登陆页面
                        resp.sendRedirect("http://127.0.0.1:8848/examsysBefore/login.html");
                        return;
                    }
                }
            }else{
